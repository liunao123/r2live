...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_path = n.advertise<nav_msgs::Path>("path", 100000);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 100000);
    // pub_relo_path = n.advertise<nav_msgs::Path>("relocalization_path", 1000);
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", 100000);
    // pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>("history_cloud", 1000);
    // pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", 1000);
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("camera_pose", 100000);
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 100000);
...
void pub_LiDAR_Odometry(const Estimator &estimator, const StatesGroup &state, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // 会覆盖已经有的文件，新建一个文件
        static std::ofstream r2live_odom_temp("/home/map/body_in_world_image_time.txt", ios::out);

        // 往上面的文件里写 打开
        std::ofstream r2live_odom("/home/map/body_in_world_image_time.txt", ios::app);

        r2live_odom.setf(ios::fixed, ios::floatfield);
        r2live_odom.precision(10);
        r2live_odom << header.stamp.toSec() << " ";
        r2live_odom.precision(5);
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // ROS_INFO("r2live time is : %lf ", header.stamp.toSec());
