		...
		else
		{
			...
			for (size_t j = 0; j < loop_pose.size() - 1; j++)
			{
				...
				if (pcd_time > before_time && pcd_time < after_time)
				{
					...
					// Eigen::Vector3d now_pcd_tans_next(lio_pose[i+1].pose.position.x, lio_pose[i+1].pose.position.y, lio_pose[i+1].pose.position.z);

					// if(( now_pcd_tans_next - now_pcd_tans).norm() > 2)
					// {

					// 	PCL_ERROR("eeeeeeeeeeee %d ", i);
