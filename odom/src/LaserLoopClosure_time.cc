...
loop_cnts_(0),
detect_time_regional_(3), // 在视觉回环发生的多少时间范围内，进行 点云匹配
...
bool LaserLoopClosure::LoadParameters() 
{
    ...
    max_tolerable_fitness_ = 0.72 ; //0.36;
    skip_recent_poses_ = 10; // 20
    poses_before_reclosing_ = 10;
    maxLoopKeysYawM = 0.8; //1.05;
                                  ...
                                  double* fitnessReturn) 
{
    ...
    *delta = gu::PoseInverse(delta_icp);
    ...
    // icp的结果 
	cout << "icp rotation (deg) :" << endl << delta->rotation.GetEulerZYX() * 180 / M_PI << endl;
...
bool LaserLoopClosure::FindLoopClosures(unsigned int key_temp, const double & target_time)
{
    ...
    for (const auto& keyed_pose : values_)
    {
        ...
        // if (other_key < 5 || other_key > key - skip_recent_poses_ - 5 )
        // {
        // }
        ...
        {      
            ...
            // 时间戳是不是在视觉回环的附近
            if( fabs(scan2->header.stamp - target_time) > detect_time_regional_ )
            {
                continue;
            }
...
void LaserLoopClosure::loopClosureThread()
{
    ...
    while(1)
    {
        ...
        for (int in = 0; in < loop_time_.size(); in++)
        {
            ...
            if (fabs(loop_time_[in].first - scan->header.stamp) < detect_time_regional_ )
            {
        ...
        {
            dzlog_info(" this key:%u time near have loop ......", iKey);
    ...
    dzlog_info(  "-------------------------------------------------------------------------  " );
    dzlog_info(  "-----------------press 's' to end main() and save result-----------------  " );
    dzlog_info(  "-------------------------------------------------------------------------  " );
